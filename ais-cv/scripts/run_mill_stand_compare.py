#!/usr/bin/env python3
"""
Mill Stand - Peak vs 2-Line Comparison (Camera 3)
==================================================
...
Press S to save positions to settings.yaml.
...
Usage:
  python scripts/run_mill_stand_compare.py
  python scripts/run_mill_stand_compare.py --channel 3
  python scripts/run_mill_stand_compare.py --brightness-threshold 160
  python scripts/run_mill_stand_compare.py --min-peak-pixels 500
  python scripts/run_mill_stand_compare.py --min-peak-duration 0.2
  python scripts/run_mill_stand_compare.py --min-travel-time 0.3
  python scripts/run_mill_stand_compare.py --sequence-timeout 4.0
"""
...
class TwoLineDetector:
    ...
    def update(
        ...
        # --- Max dwell-time gate ---
        # Track how long each line has been continuously triggered.
        # If it exceeds max_dwell_time, force-reset that line's consec counter
        # so it can never confirm — it's a person or slow obstruction, not a piece.
        if l1_trig:
            if self._l1_dwell_start is None:
                self._l1_dwell_start = timestamp
            elif timestamp - self._l1_dwell_start > self.max_dwell_time:
                l1_trig = False   # suppress — dwell too long
        ...
        if l2_trig:
            if self._l2_dwell_start is None:
                ...
                l2_trig = False   # suppress — dwell too long
...
def save_positions(roi: ROI, l1: LineSeg, l2: LineSeg):
    ...
    # Also update mill_stand.zones bounding box for peak ROI
    if "mill_stand" not in config:
        config["mill_stand"] = {}
    config["mill_stand"]["peak_roi"] = {
...
def print_help():
    print("""
Mill Stand Compare - Controls
-------------------------------
  Q / ESC      Quit
  Space        Pause / resume
  R            Reset both counts
  S            Save ROI + line positions to settings.yaml
  H            This help
...
def run(args):
    ...
    # --- Load saved positions from settings.yaml (View 3) if available ---
    views = config.get("mill_stand_lines", {}).get("views", [])
    v3 = views[2] if len(views) >= 3 else {}

    # ROI default: centre of frame, ~40% width x 30% height
    saved_roi = config.get("mill_stand", {}).get("peak_roi", None)
    if saved_roi:
    ...
    # Line 1 default: left-third of ROI, vertical
    if v3.get("line1"):
        lc = v3["line1"]
    ...
    # Line 2 default: right-third of ROI, vertical
    if v3.get("line2"):
        lc = v3["line2"]
    ...
    line_det = TwoLineDetector(
        ...
        csv_path=PROJECT_ROOT / "logs" / "line_brightness_cam3.csv",
    )
    ...
    # --- Window ---
    WIN = "Mill Stand Compare - Camera 3  |  A: Peak  |  B: 2-Line"
...
def main():
    p = argparse.ArgumentParser(
        description="Side-by-side Peak vs 2-Line mill stand counter (Camera 3)"
    )
